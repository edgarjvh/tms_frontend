    ...
    create(callback) {
        ...
        if ($(document).find('.swiper-slide #customer-container').length === 0) {
            ...
            $.get(location + 'views/customer/customer.html', async function (content) {
                ...
                $.post(serverURL + '/notes').then(async res => {
                    console.log(res);
...
function eventListeners() {
    ...
    $(document).on('click', '.panel-contact-container .contact-info-item', function (e) {
        ...
        $.post(serverURL + '/getContactById', {
            ...
            .then(res => {
                ...
                contactInfoContainer.find('.contact-info-header-data-extra-occupation').text(con.title);
    ...
    $(document).on('click', '.btn-panel-contacts-save', function (e) {
        ...
        if (email_work.val().trim() === '' && email_personal.val().trim() === '' && email_other.val().trim() === '') {
            alert('Must enter at least one email');
            return;
        }
        if (phone_work.val().trim() === '' && phone_work_fax.val().trim() === '' && phone_mobile.val().trim() === '' && phone_direct.val().trim() === '' && phone_other.val().trim() === '') {
...
function validateCustomerForSaving() {
    ...
    if (name.val().trim().replace(/\s/g, '').replace('&', 'A') !== '' &&
