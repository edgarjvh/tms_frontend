...
export class AdminReports {
    create(callback) {
        console.log('creating admin reports');
        let container = $(document).find('#admin-main-container .swiper-wrapper');

        if ($(document).find('.swiper-slide #admin-reports-container').length === 0) {
            let loader = $(document).find('.main-app-loader');
            loader.fadeIn(300);
            $.get(location + 'views/admin-reports/admin-reports.html', async function (content) {
                $(container).append(content);
                await eventListeners();
                await reorderAdminReportsPanels();
                await callback();
                loader.fadeOut(300);
            }, 'html');
        }
...
function eventListeners() {
    let containerWidth = $(document).find('#swiper-slide-admin-reports').width();
 
    $('#admin-reports-main-panel-container')
        .resizable({
            handles: "w",
            containment: "parent",
    ...
    $(document).on('click focus', '#admin-main-container .input-box-container.drop-down input, .input-box-container.drop-down span', function (e) {
        ...
        let popupContainer = input.closest('.swiper-wrapper').find('#admin-reports-mochi-contextual-container');
...
function reorderAdminReportsPanels() {
    let mainContainer = $(document).find('#admin-reports-main-panel-container');
