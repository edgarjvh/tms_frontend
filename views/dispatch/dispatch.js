...
export class DispatchContainer {
    create(callback) {
        ...
        if ($(document).find('.swiper-slide #dispatch-container').length === 0) {
            ...
            loader.fadeIn(300);
            $.get(location + 'views/dispatch/dispatch.html', function (content) {
                $.post(serverURL + '/getDispatchNotes').then(async res => {

                    if (res.result === 'OK') {
                        let notesForCarrierHtml = ``;
                        let internalNotesHtml = ``;

                        for (let i = 0; i < res.notes_for_carrier.length; i++) {
                            let note = res.notes_for_carrier[i];

                            notesForCarrierHtml += `
                            <div 
                                class="notes-for-carrier-list-item" 
                                data-id="${note.id}" 
                                data-user="${note.user}" 
                                data-note="${note.note}" 
                        ...
                        content = content.replace('[NOTES_FOR_CARRIER_LIST]', notesForCarrierHtml);
                        content = content.replace('[INTERNAL_NOTES_LIST]', internalNotesHtml);                       
                    }
...
function eventListeners() {
    ...
    $(document).on("click", "#dispatch-notes-for-carrier-add-note-btn", function (e) {
        ...
        let modal = dispatchContent.find(".modal-dispatch-notes-for-carrier");
        modal.attr("class", "modal-dispatch-notes-for-carrier adding");
        modal.find("textarea").val("");
        modal.fadeIn();
        modal.find("textarea").focus();
    });

    $(document).on("click", "#dispatch-notes-for-carrier-delete-btn", function (e) {
        let dispatchContent = $(this).closest(".dispatch-content");
        let notesForCarrierPortalSectionWrapper = dispatchContent.find(".notes-for-carrier-portal-section-wrapper");

        if (confirm('Are you sure to delete this note?')) {
            let modal = dispatchContent.find(".modal-dispatch-notes-for-carrier");
            let noteId = modal.attr('data-id');

            $.post(serverURL + '/deleteNotesForCarrier', {
                id: noteId
            })
                .then(res => {
                    let notesList = ``;
                    ...
                    for (let i = 0; i < res.notes_for_carrier.length; i++) {
                        notesList += `
                                <div 
                                    class="notes-for-carrier-list-item" 
                                    data-id="${res.notes_for_carrier[i].id}" 
                                    data-user="${res.notes_for_carrier[i].user}" 
                                    data-note="${res.notes_for_carrier[i].note}" 
                                    data-datetime="${res.notes_for_carrier[i].date_time}">
                                        ${res.notes_for_carrier[i].note}
                                </div>
                                `;
                    }

                    notesForCarrierPortalSectionWrapper.html(notesList);
                    $("#dispatch-notes-for-carrier-cancel-btn").click();
                })

        }
    });

    $(document).on("click", "#dispatch-notes-for-carrier-edit-btn", function (e) {
        let dispatchContent = $(this).closest(".dispatch-content");

        let modal = dispatchContent.find(".modal-dispatch-notes-for-carrier");
        modal.attr("class", "modal-dispatch-notes-for-carrier editing");
        modal.find('textarea').val(modal.attr('data-note'));
        modal.find("textarea").focus();
    });

    $(document).on("click", "#dispatch-notes-for-carrier-cancel-btn", function (e) {
        let dispatchContent = $(this).closest(".dispatch-content");
        let modal = dispatchContent.find(".modal-dispatch-notes-for-carrier");
    ...
    $(document).on("click", "#dispatch-notes-for-carrier-save-btn", function (e) {
        let modal = $(this).closest(".modal-dispatch-notes-for-carrier");
        let noteId = Number(modal.attr('data-id'));
        let modalContainer = modal.find(".modal-dispatch-notes-for-carrier-content");
        ...
        $.post(serverURL + "/saveNotesForCarrier", {
            id: noteId,
    ...
    $(document).on("click", ".notes-for-carrier-list-item", function (e) {
        ...
        modal.attr('data-user', noteUser);
        modal.attr('data-datetime', noteDateTime);
        modal.attr('data-note', noteText);

        let textarea = modal.find("textarea");
        modal.attr("class", "modal-dispatch-notes-for-carrier showing");
    ...
    $(document).on("click", ".internal-notes-list-item", function (e) {
        ...
        let textarea = modal.find("textarea");
        modal.attr("class", "modal-dispatch-internal-notes showing");
    ...
    $(document).on('keyup', '#txt-carrier-insurance-company', function (e) {
        ...
        let input = $(this);
        let key = e.keyCode || e.which;
        let popupContainer = $(document).find('.mochi-contextual-container');
        let popup = popupContainer.find('.mochi-contextual-popup');
