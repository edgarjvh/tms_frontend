...
const EmployeeSearch = (props) => {
    const refEmployeeSearchContainer = useRef();
                ...
                let panel = {
                    ...
                    component: <Employees
                        ...
                        closingCallback={() => {
                            ...
                            refEmployeeSearchContainer.current.focus({ preventScroll: true });
                        }}

                        employeeSearchCompany={{
                            ...x.company,
                            selectedEmployee: ((x.company.employees || []).find(employee => employee.id === x.id) || {})
                        }}
                    />
                }
    ...
    useEffect(() => {
        let url = '/companyEmployeesSearch';

        setIsLoading(true);
        axios.post(props.serverUrl + url, props.employeeSearch).then(async res => {
            if (res.data.result === 'OK') {
                setEmployees(res.data.employees);
            }
            setIsLoading(false);
        }).catch(e => {
            console.log('error searching employees', e);
            setIsLoading(false);
        }).finally(() => {
            refEmployeeSearchContainer.current.focus({ preventScroll: true });
        });
    ...
    return (
        <div className="panel-content" ref={refEmployeeSearchContainer} tabIndex={0} onKeyDown={e => {
            if (e.key === 'Escape'){
                e.stopPropagation();
            ...
            <div className="input-box-container" style={{ marginTop: 20, display: 'flex', alignItems: 'center' }}>
                {
                    (props.employeeSearch.filters || []).map((item, index) => {
