...
const AgentSearch = (props) => {
    const refAgentSearchContainer = useRef();
                ...
                let panel = {
                    ...
                    component: <Agents
                        ...
                        closingCallback={() => {
                            ...
                            refAgentSearchContainer.current.focus({ preventScroll: true });
                        }}

                        agentSearchCompany={{
                            ...x.company,
                            selectedAgent: ((x.company.agents || []).find(agent => agent.id === x.id) || {})
                        }}
                    />
                }
    ...
    useEffect(() => {
        let url = '/companyAgentsSearch';

        setIsLoading(true);
        axios.post(props.serverUrl + url, props.agentSearch).then(async res => {
            if (res.data.result === 'OK') {
                setAgents(res.data.agents);
            }
            setIsLoading(false);
        }).catch(e => {
            console.log('error searching agents', e);
            setIsLoading(false);
        }).finally(() => {
            refAgentSearchContainer.current.focus({ preventScroll: true });
        });
    ...
    return (
        <div className="panel-content" ref={refAgentSearchContainer} tabIndex={0} onKeyDown={e => {
            if (e.key === 'Escape') {
                e.stopPropagation();
            ...
            <div className="input-box-container" style={{ marginTop: 20, display: 'flex', alignItems: 'center' }}>
                {
                    (props.agentSearch.filters || []).map((item, index) => {
