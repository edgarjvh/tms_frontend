...
import './ReportCarrierOpenInvoices.css';
...
const ReportCarrierOpenInvoices = (props) => {
  const refCarrierOpenInvoicesContainer = useRef();
  ...
  const [equipment, setEquipment] = useState({ id: -1, name: 'All' });
  ...
  const [url, setUrl] = useState('/getCarrierOpenInvoicesReport');
    ...
    axios.post(props.serverUrl + url, {
      ...
      if (res.data.result === 'OK') {
        ...
        newOrders.map((order, index) => {
          ...
          if (lastCarrier.id === 0) { // if is the first carrier
            lastCarrier = currentCarrier;
            ...
            if (currentCarrier.id === lastCarrier.id) { // if is the same carrier than before
      ...
      console.log('Error on getting orders by carriers', e);
    }).finally(() => {
      setIsLoading(false);
  ...
  const doSearch = (equipment_id = null) => {
    ...
    axios.post(props.serverUrl + url, {
      ...
      equipment_id: equipment_id || equipment.id,
      date_start: dateStart,
      date_end: dateEnd,
      city_origin: (cityOrigin || '').trim(),
      city_destination: (cityDestination || '').trim(),
  ...
  const handleExport = () => {
    if (orders.length > 0) {
      ...
      orders.map((item1, index1) => {
        const { id, code, code_number, name, city, state } = item1.carrier;
  ...
  return (
    <div className="panel-content" tabIndex={0} ref={refCarrierOpenInvoicesContainer} onKeyDown={e => {
      if (e.key === 'Escape') {
        e.stopPropagation();
        if ((dateStart || '') !== '' ||
          (dateEnd || '') !== '' ||
      ...
      <div className="order-fields-container report-carrier-open-invoices">
        ...
        <div className="search-fields">
          ...
          <div className="input-box-container">
            <div style={{ fontSize: '0.7rem', color: 'rgba(0,0,0,0.7)', whiteSpace: 'nowrap' }}>Carrier Code
            </div>
            <input type="text"
          ...
          <div className="select-box-container" style={{ flexGrow: 1 }}>
            <div className="select-box-wrapper">
              <div style={{ fontSize: '0.7rem', color: 'rgba(0,0,0,0.7)', whiteSpace: 'nowrap' }}>Equipments</div>
              <input
                type="text"
                  ...
                  switch (key) {
                    ...
                    case 38: // arrow left | arrow up
                      ...
                      if (equipmentItems.length > 0) {
                        ...
                        axios.post(props.serverUrl + "/getEquipments", { withAll: 1 }).then((res) => {
                          if (res.data.result === "OK") {
                            setEquipmentItems(res.data.equipments.map((item) => {
                              item.selected = item.id === (equipment?.id || -1);
                              return item;
                            }));
                    ...
                    case 13: // enter
                      if (equipmentItems.length > 0 && equipmentItems.findIndex((item) => item.selected) > -1) {
                        setEquipment({
                          id: equipmentItems[equipmentItems.findIndex((item) => item.selected)].id,
                          name: equipmentItems[equipmentItems.findIndex((item) => item.selected)].name
                        });

                        setEquipmentItems([]);
                        window.setTimeout(() => {
                          doSearch(equipmentItems[equipmentItems.findIndex((item) => item.selected)].id);
                        }, 10)
                      }
                      break;
                ...
                onInput={(e) => {
                  ...
                  if (e.target.value.trim() === "") {
                    ...
                    axios.post(props.serverUrl + "/getEquipments", { withAll: 1, name: e.target.value.trim(), }).then((res) => {
                      if (res.data.result === "OK") {
                        setEquipmentItems(res.data.equipments.map((item) => {
      ...
      <div className="order-info-container report-carrier-open-invoices">
...

          {
            ...
            <div className="order-info-header">
              <div className="order-info-header-wrapper">
                <div className="order-info-col equipment">Equipment</div>
                ...
                <div className="order-info-col invoice-received-date">Invoice Rec'd Date</div>
              </div>
            </div>
          }
            ...
            <div className="order-info-wrapper">
              {
                (orders || []).map((groupOrder, index1) => {
                  const { id, code, name } = groupOrder.carrier;
                  ...
                  return (
                    <div className="customer-container" key={index1}>
                      <div className="customer-info">
                        <span>Carrier</span>
                        <span className="carrier-code" onClick={() => {
                                        ...
                                        <div className="month-container"
                                              ...
                                              orders.map((order, index4) => {
                                                ...
                                                return (
                                                  <div
                                                    ...
                                                    <div className="order-info-col equipment">
                                                    ...
                                                    <div className="order-info-col invoice-received-date">
                                                      {
                                                        order?.invoice_received_date || ''
                                                      }
                                                    </div>
                                                  </div>
                                                )
...

                                          {
                                            ...
                                            <div className="month-totals">
                                              ...
                                              <div className="order-info-col invoice-received-date"></div>
                                            </div>
                                          }
                                        </div>
                                      )
